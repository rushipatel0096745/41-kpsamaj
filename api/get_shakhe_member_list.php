...
function main($json)
{
	...
	$shakhemembersql = "SELECT fm.id,fm.family_id,fm.surname,fm.name,lec.course_name FROM family_member fm INNER JOIN family_registration fr ON fr.family_id = fm.family_id inner join native n ON n.id = fr.native inner Join shakhe sh ON sh.id = fr.shakhe LEFT JOIN form_educations fe ON fe.member_id = fm.id LEFT JOIN list_education_course lec ON lec.id = fe.education_course   WHERE sh.native_id='" . $data['village_id'] . "' and sh.id='" . $data['shakhe_id'] . "'";

	$shakhememberposts = $obj->select($shakhemembersql);
	
	if ($shakhememberposts === false || $shakhememberposts === null) {
		return [
			"code"    => "500",
			"success" => "0",
