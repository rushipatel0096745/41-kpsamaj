...
function main($json) {
    ...
    // Query
    $query = "SELECT id, dob FROM family_member WHERE (mobile = '$username' OR email = '$username') AND TIMESTAMPDIFF(YEAR, dob, CURDATE()) >= 18;";
    ...
    if (!empty($listData) && count($listData) > 0) {
        ...
        $response = array(
            "code" => "200",
            "success" => "0",
            "message" => "Member already created with the same mobile number",
            "data" => ""
        );
    } else {
