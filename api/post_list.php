...
LEFT JOIN family_member fm_login ON fm_login.id = '".$data['member_id']."' LEFT JOIN family_registration fr_login ON fr_login.family_id = fm_login.family_id WHERE (p.visibility = 'Village' AND fr_post.native = fr_login.native) OR p.visibility = 'Public' GROUP BY p.id ORDER BY p.createdon DESC LIMIT 10 OFFSET ".$data['page']."";
